...
# --- 1. 頁面配置 ---
st.set_page_config(page_title="建築工期估算系統 v3.9", layout="wide")
...
excavation_map = {
    "連續壁 + 型鋼內支撐 (標準)": 1.0,
    "連續壁 + 地錨 (開挖動線佳)": 0.9,     # 地錨出土快
    "全套管切削樁 + 型鋼內支撐": 0.95,     # 比連續壁略快
    "預壘樁/排樁 + 型鋼內支撐": 0.85,      # 工法本身較快
    "鋼板樁 + 型鋼內支撐 (淺開挖)": 0.7,   # 極快
    "放坡開挖/無支撐 (極快)": 0.5          # 最快
}
...
# 公式：(基本層數天數 * 開挖系統係數 + 基礎加成) * 面積係數
d_sub = int(((base_sub_days * excav_multiplier) + foundation_add) * area_multiplier)
...
schedule_data = [
    ...
    {"工項階段": "4. 基礎/地下室工程", "需用工作天": d_sub, "開始日期": p3_start, "完成日期": p3_end, "備註": f"要徑 ({excavation_system})"},
...
report_rows = [
    ...
    ["基礎型式", foundation_type], ["開挖擋土系統", excavation_system], ["地質改良", soil_improvement],
