...
# --- 1. 頁面配置 ---
st.set_page_config(page_title="建築工期估算系統 v3.5", layout="wide")
...
with st.expander("點擊展開/隱藏 建築規模與基地資訊", expanded=True):
    ...
    with col1:
        ...
        foundation_type = st.selectbox("基礎型式 (New)", ["筏式基礎 (標準)", "樁基礎 (工期較長)", "獨立基腳"])
...
# 4. 地下室/基礎工程 (New: 納入基礎型式影響)
# 樁基礎通常比筏式基礎多出打樁時間 (約 45-60 天視面積而定)
foundation_add = 60 if "樁基礎" in foundation_type else 0
d_sub = int((floors_down * (45 if b_method == "順打工法" else 55) + foundation_add) * area_multiplier)
...
schedule_data = [
    ...
    {"工項階段": "2. 建物拆除與整地", "需用工作天": d_demo, "開始日期": p2_start, "完成日期": p2_end, "備註": "要徑 (若無則跳過)"},
    {"工項階段": "3. 地質改良工程", "需用工作天": d_soil, "開始日期": p_soil_start, "完成日期": p_soil_end, "備註": "要徑 (開挖前)"},
    {"工項階段": "4. 基礎/地下室工程", "需用工作天": d_sub, "開始日期": p3_start, "完成日期": p3_end, "備註": f"要徑 ({foundation_type})"},
    {"工項階段": "5. 地上主體結構", "需用工作天": d_super, "開始日期": p4_start, "完成日期": p4_end, "備註": "要徑"},
    {"工項階段": "6. 內裝機電/管線", "需用工作天": d_mep, "開始日期": p5_start, "完成日期": p5_end, "備註": "結構 30% 進場"},
    {"工項階段": "7. 室內裝修/景觀", "需用工作天": d_finishing, "開始日期": p6_start, "完成日期": p6_end, "備註": "結構 60% 進場"},
...
report_rows = [
    ...
    ["基礎型式", foundation_type], ["地質改良", soil_improvement],
